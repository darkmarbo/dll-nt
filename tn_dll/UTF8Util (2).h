...
//	~UTF8Util(void);
	...
	*/
	static size_t StrCharLength(const char* str)
	{
		...
		//const char* pbuff = FindNextInline(inbuff, '\t');	//不是UTF-8编码的，在此卡住了。。。。。20151124
		...
		const char* inbuff = str;
		while(!IsLineEndingOrFileEnding(*inbuff))
		{
			inbuff = NextChar(inbuff);
			length_count += 1;
		}
	...
	*/
	...
	{
		//size_t length_count = 0; //yqw 20151124 修改
		...
		const char* inbuff = str;
		//const char* pbuff = FindNextInline(inbuff, '\t');
		while(!IsLineEndingOrFileEnding(*inbuff) && length_count<length)
		{
			inbuff = NextChar(inbuff);
			length_count += 1;
	...
	*/
	static size_t NextCharLengthNoException(const char* str) {
		...
		//return 0; //
		return 1;   // yqw 20151124修改 乱码时进入死循环
	}
	...
	*/
	static size_t NextCharLength(const char* str) {
		...
		if (length == 0) {
			...
			length = 1; // yqw 20151124 添加，乱码时进入死循环
		}
		return length;
	}
